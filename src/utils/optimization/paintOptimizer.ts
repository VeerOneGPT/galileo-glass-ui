...
export class PaintOptimizer {
  ...
  private scrollListenerActive: boolean = false;
  ...
  public optimizeElement(
    element: HTMLElement,
    isGlassComponent: boolean = false
  ): void {
    if (!element) return;
  ...
  public optimizeForScroll(active: boolean): void {
    ...
    if (active) {
      ...
      document.querySelectorAll('[data-glass-component="true"][data-scroll-optimized="true"]').forEach(element => {
  ...
  private analyzeElement(
    ...
    const hasComplexBorder = style.borderImage !== 'none' ||
    ...
    const isAnimating = element.classList.contains('is-animating') ||
      style.animation !== 'none' ||
      style.transition !== 'none' ||
      this.animatingElements.has(element);
    
    const hasComplexBackground = style.backgroundImage !== 'none' || 
  ...
  private determineOptimizationActions(
    attributes: GlassPaintAttributes
  ): PaintOptimizationActions {
    ...
    if (this.isScrolling || 
        ...
        (attributes.hasBackdropFilter || attributes.hasBoxShadow || attributes.hasComplexBackground)) {
  ...
  private checkScrollStop = (): void => {
    ...
    if (timeSinceLastScroll > 100) { // 100ms threshold
 ...
 */
export const globalPaintOptimizer = typeof window !== 'undefined' 
  ? new PaintOptimizer()
  : null;
 ...
 */
export const createPaintOptimizer = (
  config?: PaintOptimizationConfig
): PaintOptimizer => {
 ...
 */
export const optimizeForPainting = (
  element: HTMLElement,
  isGlassComponent: boolean = false
): void => {
  if (globalPaintOptimizer) {
