...
export class OptimizedStyleSheet {
  ...
  private createStyleSheet(): HTMLStyleElement {
    ...
    if (this.config.includeComments) {
      style.textContent = `/* Galileo Glass UI - Optimized StyleSheet #${this.styleSheets.length + 1} */\n`;
    }
  ...
  private insertRule(className: string, css: string): void {
    ...
    let targetSheet = this.findTargetStyleSheet();
 ...
 */
export const globalStyleSheet = typeof window !== 'undefined'
