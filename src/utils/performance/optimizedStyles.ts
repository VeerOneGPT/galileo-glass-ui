...
export const getOptimizedTransforms = (transforms: { [key: string]: string | number }) => {
  ...
  if (transforms.translateX) {
    transformValues.push(`translateX(${transforms.translateX}${typeof transforms.translateX === 'number' ? 'px' : ''})`);
  }
  
  if (transforms.translateY) {
    transformValues.push(`translateY(${transforms.translateY}${typeof transforms.translateY === 'number' ? 'px' : ''})`);
  }
  
  if (transforms.translateZ) {
    transformValues.push(`translateZ(${transforms.translateZ}${typeof transforms.translateZ === 'number' ? 'px' : ''})`);
  }
  ...
  if (transforms.rotate) {
    transformValues.push(`rotate(${transforms.rotate}${typeof transforms.rotate === 'number' ? 'deg' : ''})`);
  }
  
  if (transforms.rotateX) {
    transformValues.push(`rotateX(${transforms.rotateX}${typeof transforms.rotateX === 'number' ? 'deg' : ''})`);
  }
  
  if (transforms.rotateY) {
    transformValues.push(`rotateY(${transforms.rotateY}${typeof transforms.rotateY === 'number' ? 'deg' : ''})`);
  }
