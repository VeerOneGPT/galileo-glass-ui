...
export class StyleCache {
  ...
  public createKey(
    ...
    const { include, exclude, sortProps = true, keyPrefix = '' } = options;
    ...
    if (sortProps) {
      ...
      keyParts = Object.entries(processedProps).map(([key, value]) => `${key}:${value}`);
    }
  ...
  private enforceComponentLimits(componentType: string): void {
    ...
    const componentEntries: [string, StyleCacheEntry][] = [];
  ...
  private updateHitRate(): void {
    ...
    this.stats.hitRate =
      totalAccesses > 0 ? Math.round((this.stats.cacheHits / totalAccesses) * 100) : 0;
  }
}
...
export function memoizeStyle<T extends Record<string, any>>(
  ...
  const { componentType, isGlass = false, cache = globalStyleCache, keyOptions = {} } = options;
  ...
  return (props: T): string => {
    ...
    return cache.getOrSet(cacheKey, () => styleGenerator(props), componentType, isGlass, props);
  };
}
