 ...
 */
export const WILL_CHANGE_CANDIDATES = [
  'transform',
  'opacity',
  'backdrop-filter',
  'filter'
 ...
 */
const TIER_SPECIFIC_RULES: Record<DeviceCapabilityTier, Record<string, PropertyOptimizationStrategy>> = {
 ...
 */
const LEVEL_SPECIFIC_RULES: Record<OptimizationLevel, Record<string, PropertyOptimizationStrategy>> = {
...
export class StyleOptimizer {
  ...
  public optimizeBoxShadow(value: string): string {
    ...
    if (value.includes(',')) {
      ...
      if (simplifiedShadows.length < shadows.length) {
        ...
        simplifiedShadows = simplifiedShadows.map(shadow => {
          ...
          const parts = shadow.match(/([^)]+\))?\s*([-\d.]+)px\s+([-\d.]+)px\s+([-\d.]+)px(\s+([-\d.]+)px)?\s+([^)]+)$/);
    ...
    if (this.config.optimizationLevel === OptimizationLevel.MAXIMUM ||
      ...
      const parts = value.match(/([^)]+\))?\s*([-\d.]+)px\s+([-\d.]+)px\s+([-\d.]+)px(\s+([-\d.]+)px)?\s+([^)]+)$/);
      ...
      if (parts) {
        ...
        if (spread !== 0) {
          return `${colorPrefix} ${xOffset}px ${yOffset}px ${newBlur}px ${Math.min(spread, 2)}px ${color}`;
        } else {
          return `${colorPrefix} ${xOffset}px ${yOffset}px ${newBlur}px ${color}`;
        }
      }
  ...
  public optimizeBackgroundOpacity(value: string): string {
    ...
    if (this.config.deviceTier === DeviceCapabilityTier.LOW ||
    ...
    } else if (this.config.deviceTier === DeviceCapabilityTier.MINIMAL ||
  ...
  public optimizeBorder(value: string, property: string): string {
    ...
    if (this.config.deviceTier === DeviceCapabilityTier.MINIMAL ||
        this.config.optimizationLevel === OptimizationLevel.MAXIMUM) {
      ...
      if (value.includes(',') || value.includes('gradient')) {
        if (property === 'border') {
          ...
          return property.includes('width') ? '1px' : 
                 property.includes('style') ? 'solid' : 
                 property.includes('color') ? 'rgba(255, 255, 255, 0.2)' : 
                 value;
        }
      }
    }
   ...
   */
  private optimizeRules(rules: { property: string; value: string }[]): { property: string; value: string }[] {
    ...
    return rules.filter(rule => {
      ...
      switch (strategy) {
        ...
        case PropertyOptimizationStrategy.SIMPLIFY:
          ...
          if (rule.property === 'box-shadow') {
            rule.value = this.optimizeBoxShadow(rule.value);
          } else if (rule.property === 'backdrop-filter' || rule.property === '-webkit-backdrop-filter') {
            rule.value = this.optimizeBackdropFilter(rule.value);
   ...
   */
  private addHardwareAcceleration(rules: { property: string; value: string }[]): { property: string; value: string }[] {
   ...
   */
  private addGlassHardwareAcceleration(rules: { property: string; value: string }[]): { property: string; value: string }[] {
    if (!this.config.useGpuAcceleration) {
      return rules;
    }
    ...
    if (this.config.useWillChange) {
      ...
      if (!hasWillChange) {
        ...
        const hasBackdropFilter = rules.some(rule => 
          rule.property === 'backdrop-filter' || rule.property === '-webkit-backdrop-filter'
        );
   ...
   */
  private groupProperties(rules: { property: string; value: string }[]): { property: string; value: string }[] {
    ...
    rules.forEach(rule => {
      ...
      } else if (rule.property.startsWith('border-') && !rule.property.includes('radius') && !rule.property.includes('image')) {
    ...
    if (backgrounds.color && backgrounds.image) {
      grouped.push({ 
        property: 'background', 
        value: `${backgrounds.color} ${backgrounds.image} ${backgrounds.position || '0 0'} ${backgrounds.repeat || 'no-repeat'}`
      });
    } else {
  ...
  private simplifyAnimation(value: string): string {
    ...
    if (this.config.disableTransitionsOnLowEnd && 
        (this.config.deviceTier === DeviceCapabilityTier.MINIMAL || 
         this.config.optimizationLevel === OptimizationLevel.MAXIMUM)) {
      return 'none';
    }
        ...
        if (value.includes('linear-gradient')) {
          ...
          if (match) {
            ...
            if (parts.length > 4) {
              ...
              const simplified = `linear-gradient(${parts[0]}, ${parts[1]}, ${parts[parts.length - 1]})`;
              return simplified;
            }
          }
          ...
          if (match) {
            ...
            if (parts.length > 4) {
              ...
              const simplified = `radial-gradient(${parts[0]}, ${parts[1]}, ${parts[parts.length - 1]})`;
              return simplified;
            }
          }
        }
