...
import { detectFeatures, FeatureLevel, getFeatureSupportLevel, GLASS_REQUIREMENTS } from '../browserCompatibility';
...
const createEmptySnapshot = (): PerformanceSnapshot => {
  const metrics: Record<MetricType, PerformanceMetric> = {} as Record<MetricType, PerformanceMetric>;
...
export class PerformanceMonitor {
  ...
  private layoutCount: number = 0;
  private paintCount: number = 0;
  private styleCount: number = 0;
  ...
  public shouldReduceGlassEffects(): boolean {
    ...
    return (
      ...
      frameTime > 25 || 
      style > 100 ||
      this.currentSnapshot.overallScore < 60
    );
  }
  ...
  private generateRecommendations(): string[] {
    ...
    if (metrics[MetricType.FPS].severity === MetricSeverity.POOR || 
    ...
    if (metrics[MetricType.STYLE].severity === MetricSeverity.POOR || 
        metrics[MetricType.LAYOUT].severity === MetricSeverity.POOR) {
      recommendations.push('Minimize CSS complexity and DOM operations to reduce layout thrashing.');
    }
    
    // Check memory usage
    if (metrics[MetricType.MEMORY].severity === MetricSeverity.POOR || 
        metrics[MetricType.JS_HEAP].severity === MetricSeverity.POOR) {
      recommendations.push('Optimize memory usage by removing unnecessary elements and cleaning up unused resources.');
    }
    ...
    if (deviceTier === DeviceCapabilityTier.LOW || deviceTier === DeviceCapabilityTier.MINIMAL) {
      ...
      recommendations.push('Increase background opacity to reduce transparency rendering cost (0.5-0.7 recommended).');
      recommendations.push('Disable glow effects and other decorative elements for low-end devices.');
    }
    
    if (metrics[MetricType.FPS].value < 40) {
      recommendations.push('Apply hardware acceleration with transform: translateZ(0) and will-change: transform to glass elements.');
    ...
    if (metrics[MetricType.DOM_NODES].severity === MetricSeverity.POOR) {
      recommendations.push('Reduce the number of DOM nodes in the document to improve rendering performance.');
    }
  ...
  private normalizeMetric(
    ...
    if (metricType === MetricType.FPS) {
      ...
      if (value >= thresholds.good) return 0.8 + (value - thresholds.good) / (thresholds.excellent - thresholds.good) * 0.2;
      if (value >= thresholds.moderate) return 0.6 + (value - thresholds.moderate) / (thresholds.good - thresholds.moderate) * 0.2;
      if (value >= thresholds.poor) return 0.3 + (value - thresholds.poor) / (thresholds.moderate - thresholds.poor) * 0.3;
    ...
    if (metricType === MetricType.FRAME_TIME) {
      if (value <= thresholds.excellent) return 1;
      if (value <= thresholds.good) return 0.8 + (thresholds.good - value) / (thresholds.good - thresholds.excellent) * 0.2;
      if (value <= thresholds.moderate) return 0.6 + (thresholds.moderate - value) / (thresholds.moderate - thresholds.good) * 0.2;
      if (value <= thresholds.poor) return 0.3 + (thresholds.poor - value) / (thresholds.poor - thresholds.moderate) * 0.3;
  ...
  private updateSnapshot(): void {
    ...
    if (timeSinceLastReset > 10000) { // Reset every 10 seconds
      this.longTaskCount = 0;
      this.layoutCount = 0;
      this.paintCount = 0;
      this.styleCount = 0;
