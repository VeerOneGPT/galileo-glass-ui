...
export const detectFeatures = (): BrowserFeatures => {
  ...
  try {
    ...
    const hasWebGL = !!(window as any).WebGLRenderingContext && 
  ...
  if ('CSS' in window && (window as any).CSS.supports) {
    features.supports.backdropFilter = (window as any).CSS.supports('backdrop-filter', 'blur(10px)');
    features.supports.webkitBackdropFilter = (window as any).CSS.supports('-webkit-backdrop-filter', 'blur(10px)');
