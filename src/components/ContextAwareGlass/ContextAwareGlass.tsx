...
const GlassContainer = styled.div<{
  ...
  ${props => props.$enableEdgeHighlight && edgeHighlight({
    thickness: 1,
    opacity: 0.5,
    position: 'all',
    themeContext: createThemeContext({}, props.$isDarkMode)
  ...
  ${props => props.$enableGlow && glassGlow({
    intensity: 'medium',
 ...
 */
export const ContextAwareGlass = forwardRef<HTMLDivElement, ContextAwareGlassProps>((props, ref) => {
  const {
    children,
    adaptationMode = 'auto',
    contentType,
  ...
  const [detectedContentType, setDetectedContentType] = useState<ContentType>(contentType || 'unknown');
  const [detectedBackgroundType, setDetectedBackgroundType] = useState<BackgroundType>(backgroundType || 'unknown');
  ...
  const detectContentType = useCallback((): ContentType => {
    ...
    const hasFormElements = container.querySelectorAll('input, select, textarea, button').length > 0;
