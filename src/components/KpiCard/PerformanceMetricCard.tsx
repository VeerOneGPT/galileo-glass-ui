...
const CardRoot = styled.div<{
  ...
  ${props => props.$highlight && !props.$reducedMotion && `
...
const GaugeProgress = styled.div<{
  ...
  width: ${props => 
    props.$size === 'small' ? '64px' : 
    props.$size === 'large' ? '96px' : 
    '80px'
  };
  height: ${props => 
  ...
  position: relative;
  border-radius: ${props => 
    props.$size === 'small' ? '32px 32px 0 0' : 
    props.$size === 'large' ? '48px 48px 0 0' : 
    '40px 40px 0 0'
  };
  background-color: rgba(255, 255, 255, 0.1);
  overflow: hidden;
      ...
      <MetricsContainer>
        <ValueContainer>
          ...
          <TargetValue $size={size}>
            Target: {formattedTarget}{unit && ` ${unit}`}
          </TargetValue>
        </ValueContainer>
 ...
 */
const GlassPerformanceMetricCard = forwardRef<HTMLDivElement, PerformanceMetricCardProps>((props, ref) => (
  <PerformanceMetricCard {...props} glass={true} ref={ref} />
));
