 ...
 */
import React, { useEffect, useRef, useState, useMemo, forwardRef, useCallback, Children } from 'react';
...
export const GlassCarousel = forwardRef<HTMLDivElement, GlassCarouselProps>((props, ref) => {
  ...
  const totalWidth = useMemo(() => {
      return itemWidth * itemCount + itemGap * (itemCount > 0 ? itemCount - 1 : 0);
  ...
  const scrollToIndex = useCallback((index: number, immediate = false) => {
    const targetIndex = loop ? index : Math.max(0, Math.min(itemCount - 1, index));
  ...
  }, [loop, itemCount, itemWidth, itemGap, setPosition, bounds, currentIndex]);
  ...
  useEffect(() => {
    ...
    return stopAutoplay;
  }, [autoplay, autoplayDelay, itemCount, isHovering, pauseOnHover, isDragging]);
