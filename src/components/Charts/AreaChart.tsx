...
const AreaSeriesContainer = styled.div`
  position: absolute;
  left: 0;
  right: 0;
  top: 0;
...
const AreaPath = styled.path<AreaPathProps>`
...
const LinePath = styled.path<LinePathProps>`
  fill: none;
...
export const AreaChart: React.FC<AreaChartProps> = ({
  ...
  return (
    <SafeChartRenderer
      ...
      backgroundColor={isDarkMode ? 'rgba(15, 23, 42, 0.5)' : 'rgba(255, 255, 255, 0.1)'}
      altText={altText || `Area chart${title ? ` showing ${title}` : ''}`}
      onError={onError}
    >
      <ChartContainer
        ref={containerRef}
