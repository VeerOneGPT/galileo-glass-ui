...
const ChartContainer = styled.div<ChartContainerProps>`
  position: relative;
  min-width: ${({ minWidth }) =>
    typeof minWidth === 'number' ? `${minWidth}px` : minWidth || '300px'};
    ...
    >
      ...
      {status !== 'loading' && status !== 'error' && !isEmpty && (
        <ChartErrorBoundary onError={onError}>
          ...
          {adaptToCapabilities &&
          (deviceTier === DeviceCapabilityTier.LOW ||
      ...
      {/* Loading state */}
      {status === 'loading' && (
        <LoadingContainer>{loadingComponent || <DefaultLoadingSpinner />}</LoadingContainer>
      )}

      {/* Error state */}
      {status === 'error' && <DefaultErrorState onRetry={handleRetry} />}

      {/* Empty state */}
      {isEmpty && <React.Fragment>{emptyComponent || <DefaultEmptyState />}</React.Fragment>}
    </ChartContainer>
  );
};
