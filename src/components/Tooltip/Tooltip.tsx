...
const getTooltipPosition = (
  ...
  switch (placement) {
    ...
    case 'top-end':
      ...
      arrowTop = tooltipRect.height;
      arrowLeft = tooltipRect.width - Math.min(tooltipRect.width / 4, targetRect.width / 2) - arrowSize * 2;
      break;
    ...
    case 'left-end':
      ...
      left = targetRect.left - tooltipRect.width - offset;
      arrowTop = tooltipRect.height - Math.min(tooltipRect.height / 4, targetRect.height / 2) - arrowSize * 2;
      arrowLeft = tooltipRect.width;
      break;
...
const TooltipContent = styled.div<{
  ...
  ${props => {
    if (props.$variant === 'glass') {
      return `
        background-color: ${props.$color === 'default' ? 'rgba(31, 41, 55, 0.8)' : `${getColorByName(props.$color)}CC`};
        color: white;
    ...
    // standard
    return `
      background-color: ${props.$color === 'default' ? 'rgba(31, 41, 55, 0.9)' : getColorByName(props.$color)};
      color: white;
    `;
  }}
  ...
  ${props => props.$visible && accessibleAnimation({
    animation: fadeIn,
    duration: 0.2,
    easing: 'ease-out'
  })}
...
const TooltipArrow = styled.div<{
  ...
  /* Variant styles */
  ${props => {
    const tooltipColor = props.$color === 'default' 
      ? (props.$variant === 'glass' ? 'rgba(31, 41, 55, 0.8)' : 'rgba(31, 41, 55, 0.9)') 
      : props.$variant === 'glass' ? `${getColorByName(props.$color)}CC` : getColorByName(props.$color);
