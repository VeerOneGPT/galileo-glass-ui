...
const AccordionRoot = styled.div<{
  ...
  position: relative;
  transition: ${props =>
    props.$enableTransitions && !props.$reducedMotion
  ...
  ${props =>
    ...
    glassSurface({
      ...
      borderOpacity:
        props.$transparency === 'none'
          ? 'none'
          : props.$transparency === 'low'
          ? 'subtle'
          : props.$transparency === 'high'
          ? 'strong'
          : 'medium',
 ...
 */
function AccordionComponent(props: AccordionProps, ref: React.ForwardedRef<HTMLDivElement>) {
  const {
    children,
    className,
    style,
