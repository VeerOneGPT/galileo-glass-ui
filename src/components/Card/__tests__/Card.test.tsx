...
describe('Card Component', () => {
  ...
  test('applies different elevation levels', () => {
    ...
    renderWithTheme(
      <Card elevation={3}>Elevated Card</Card>
    );
    
    expect(glassSurface).toHaveBeenCalledWith(expect.objectContaining({
      elevation: 3
    }));
  });
  ...
  test('applies glow effect when specified', () => {
    ...
    renderWithTheme(
      <Card glow="medium" glowColor="secondary">Glowing Card</Card>
    );
    
    expect(glassGlow).toHaveBeenCalledWith(expect.objectContaining({
      intensity: 'medium',
      color: 'secondary'
    }));
  });
...
describe('GlassCard Component', () => {
  ...
  test('has different default props than regular Card', () => {
    ...
    renderWithTheme(
      <GlassCard>Default Glass Card</GlassCard>
    );
    
    // GlassCard has elevation 2 by default (vs 1 for Card)
    expect(glassSurface).toHaveBeenLastCalledWith(expect.objectContaining({
      elevation: 2
    }));
