...
const ToolbarContainer = styled.div<{
  ...
  ${props => {
    ...
    return css`
      ...
      color: ${props.$color === 'transparent' || props.$color === 'default'
        ? '#1F2937'
        : '#FFFFFF'};
      box-shadow: ${props.$elevation > 0 ? getElevationShadow(props.$elevation) : 'none'};
    `;
  ...
  ${props =>
    ...
    css`
      border-bottom: 1px solid
        ${props.$variant === 'glass' ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.05)'};
    `}
...
export const Toolbar = forwardRef<HTMLDivElement, ToolbarProps>((props, ref) => {
  ...
  return (
    <ToolbarContainer
      ...
      className={className}
      $variant={
        variant === 'standard' || variant === 'dense' || variant === 'regular'
          ? 'standard'
          : variant
      }
      $color={color}
...
export const GlassToolbar = forwardRef<HTMLDivElement, ToolbarProps>((props, ref) => {
  ...
  return (
    <Toolbar ref={ref} className={`glass-toolbar ${className || ''}`} variant={variant} {...rest} />
  );
});
