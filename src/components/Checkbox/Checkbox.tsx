...
const CheckboxControl = styled.span<{
  ...
  color: ${props =>
    props.$checked || props.$indeterminate ? 'white' : 'rgba(255, 255, 255, 0.7)'};
  border-radius: 4px;
  transition: all 0.2s ease;
  background-color: ${props =>
    props.$checked || props.$indeterminate ? getCheckboxColor(props.$color) : 'transparent'};
  border: ${props =>
...
export const GlassCheckbox = forwardRef<HTMLInputElement, CheckboxProps>((props, ref) => {
  ...
  return (
    <Checkbox ref={ref} glass={glass} className={`glass-checkbox ${className || ''}`} {...rest} />
  );
});
