...
export const generatePhysicsKeyframes = (
  ...
  const keyframesAnimation = keyframes`
    ${Object.entries(keyframePoints).map(([percent, props]) => `
      ${percent} {
        ${Object.entries(props).map(([prop, value]) => `${prop}: ${value};`).join('\n        ')}
      }
    `).join('\n')}
  `;
...
const generateElasticKeyframes = (
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    const scaleModifier = 1 + (Math.cos(frequency * Math.PI * t) * Math.exp(-decay * t) * elasticity * 0.3);
    ...
    const rotationModifier = (Math.cos(frequency * Math.PI * t) * Math.exp(-decay * t) * elasticity * 10);
...
const generateLiquidKeyframes = (
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    const xScale = 1 + (Math.sin(frequency * Math.PI * t) * Math.exp(-decay * t) * (1 - viscosity) * 0.3);
    const yScale = 1 - (Math.sin(frequency * Math.PI * t) * Math.exp(-decay * t) * (1 - viscosity) * 0.2);
    ...
    keyframePoints[percent] = {
      transform: `translate(${x}px, ${y}px) rotate(${rotation}deg) scale(${baseScale * xScale}, ${baseScale * yScale})`,
      opacity: `${opacity}`
    };
  }
...
const generateChainKeyframes = (
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    const rotation = initial.rotation + (target.rotation - initial.rotation) * rotDelay + springFactor(rotDelay) * 15;
    const scale = initial.scale + (target.scale - initial.scale) * scaleDelay * (1 + springFactor(scaleDelay) * 0.1);
 ...
 */
export const advancedPhysicsAnimation = (options: AdvancedPhysicsOptions): ReturnType<typeof css> => {
