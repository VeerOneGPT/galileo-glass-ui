 ...
 */
export type CursorEffectType = 'glow' | 'ripple' | 'trail' | 'magnetic' | 'repel' | 'spotlight';
...
export const useMouseCursorEffect = (options: MouseCursorEffectOptions = {}) => {
  ...
  const setupGlowEffect = useCallback(() => {
    ...
    if (targetSelector) {
      targetElementsRef.current = Array.from(
  ...
  const updateTrailEffect = useCallback(
    (x: number, y: number) => {
      ...
      effectElementsRef.current.forEach((particle, index) => {
        ...
        setTimeout(() => {
          ...
          particle.style.opacity = String(
            ((adjustedParticleCount - index) / adjustedParticleCount) * 0.7
          );
  ...
  const updateMagneticEffect = useCallback(
    (x: number, y: number) => {
  ...
  const updateRepelEffect = useCallback(
    (x: number, y: number) => {
      if (targetElementsRef.current.length === 0) return;
  ...
  const updateSpotlightEffect = useCallback(
    (x: number, y: number) => {
  ...
  }, [createContainer, setupEffect, handleMouseMove, handleClick, updateEffect, type]);
