...
export const edgeHighlight = (options: EdgeHighlightOptions) => {
  const { thickness = 1, opacity = 0.7, position = 'all', color = 'white', themeContext } = options;
  ...
  if (themeContext?.getColor && typeof color === 'string') {
    ...
    switch (color) {
      ...
      case 'white':
        ...
        break;
      default:
        highlightColor = typeof color === 'string' ? color : '#FFFFFF';
    }
  }
  ...
  switch (position) {
    case 'top':
      borderStyle = `border-top: ${thickness}px solid ${highlightColor}${Math.floor(
        opacity * 255
      ).toString(16)}`;
      break;
    case 'bottom':
      borderStyle = `border-bottom: ${thickness}px solid ${highlightColor}${Math.floor(
        opacity * 255
      ).toString(16)}`;
      break;
    case 'left':
      borderStyle = `border-left: ${thickness}px solid ${highlightColor}${Math.floor(
        opacity * 255
      ).toString(16)}`;
      break;
    case 'right':
      borderStyle = `border-right: ${thickness}px solid ${highlightColor}${Math.floor(
        opacity * 255
      ).toString(16)}`;
      break;
    case 'all':
    default:
      borderStyle = `border: ${thickness}px solid ${highlightColor}${Math.floor(
        opacity * 255
      ).toString(16)}`;
  }
