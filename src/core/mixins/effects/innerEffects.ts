...
export const innerGlow = (options: InnerGlowOptions) => {
  ...
  const glowIntensity = typeof intensity === 'number'
    ? intensity
    : intensity === 'subtle' ? 0.2
      : intensity === 'medium' ? 0.35
      : intensity === 'strong' ? 0.5
      : 0.35;
  ...
  const opacityHex = Math.floor(glowIntensity * 255).toString(16).padStart(2, '0');
