...
export const innerGlow = (options: InnerGlowOptions = {}) => {
  ...
  const resolvedColor = themeContext?.getColor ? 
    themeContext.getColor(color, color) : 
    color;
  
  const alpha = getIntensityAlpha(intensity);
  const glowColor = `${resolvedColor}${Math.round(alpha * 255).toString(16).padStart(2, '0')}`;
...
export const innerHighlight = (options: InnerGlowOptions = {}) => {
  ...
  const highlightColor = `${resolvedColor}${Math.round(alpha * 255).toString(16).padStart(2, '0')}`;
...
export const innerShadow = (options: InnerGlowOptions = {}) => {
  ...
  const alpha = getIntensityAlpha(intensity);
  const shadowColor = `${resolvedColor}${Math.round(alpha * 255).toString(16).padStart(2, '0')}`;
