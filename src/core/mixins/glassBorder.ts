...
export const glassBorder = (options: GlassBorderOptions) => {
  ...
  const borderColor =
    color || (isDarkMode ? 'rgba(255, 255, 255, 0.25)' : 'rgba(255, 255, 255, 0.75)');
  ...
  const gradientBorder = gradient
    ...
    border-image: ${
      isDarkMode
        ? `linear-gradient(135deg, rgba(255,255,255,${borderOpacity * 0.6}), rgba(255,255,255,${
            borderOpacity * 1.4
          }))`
        : `linear-gradient(135deg, rgba(255,255,255,${borderOpacity * 1.2}), rgba(255,255,255,${
            borderOpacity * 0.8
          }))`
    } 1;
  `
  ...
  return cssWithKebabProps`
    ${
      borders.top
        ? `border-top: ${borderWidth} ${style} ${withAlpha(borderColor, borderOpacity)};`
        : ''
    }
    ${
      borders.right
        ? `border-right: ${borderWidth} ${style} ${withAlpha(borderColor, borderOpacity)};`
        : ''
    }
    ${
      borders.bottom
        ? `border-bottom: ${borderWidth} ${style} ${withAlpha(borderColor, borderOpacity)};`
        : ''
    }
    ${
      borders.left
        ? `border-left: ${borderWidth} ${style} ${withAlpha(borderColor, borderOpacity)};`
        : ''
    }
