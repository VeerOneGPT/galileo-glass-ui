...
export const glassGlow = (props: GlowEffectProps, theme?: ThemeContext) => {
  ...
  const baseGlow = css`
    ...
    ${props.borderGlow
      ? `border: ${borderGlowSize}px solid ${glowColor}${Math.round(glowOpacity * 150).toString(
          16
        )};`
      : ''}
  `;
  ...
  if (props.hoverEffect) {
    return css`
      ...
      &:hover {
        ...
        ${props.borderGlow
          ? `border: ${borderGlowSize}px solid ${glowColor}${Math.round(glowOpacity * 180).toString(
              16
            )};`
          : ''}
      }
