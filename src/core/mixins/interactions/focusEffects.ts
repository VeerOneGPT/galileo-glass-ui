...
export const focusEffects = (options: FocusEffectsOptions) => {
  ...
  const focusColor = color || (isDarkMode 
    ? highContrast ? '#ffffff' : '#90caf9'
    : highContrast ? '#000000' : '#2196f3');
  ...
  switch (type) {
    ...
    case 'ring':
      focusStyle = `
        ${focusSelector} {
          outline: none;
          box-shadow: 0 0 0 ${offset}px ${isDarkMode ? '#000000' : '#ffffff'}, 0 0 0 ${offset + thickness}px ${withAlpha(focusColor, opacity)};
          ${animationStyles}
        }
      `;
      break;
