...
const getOptionValues = (options: InteractiveGlassOptions, isDarkMode: boolean) => {
  ...
  const getBlurValue = (blur?: string | number): string => {
    ...
    switch (blur) {
      case 'minimal': return '4px';
      case 'light': return '8px';
      case 'standard': return '12px';
      case 'enhanced': return '20px';
      default: return '12px';
    }
  };
 ...
 */
const buildBaseStyles = (values: ReturnType<typeof getOptionValues>, isDarkMode: boolean): string => {
  return `
    position: relative;
 ...
 */
const buildHoverEffects = (hoverEffect: string | undefined, values: ReturnType<typeof getOptionValues>, isDarkMode: boolean): string => {
  ...
  switch (hoverEffect) {
    case 'glow':
      hoverStyles = `
        &:hover {
          box-shadow: 0 ${values.elevation * 3}px ${values.elevation * 6}px ${withAlpha(values.color, 0.3)};
 ...
 */
const buildActiveEffects = (activeEffect: string | undefined, values: ReturnType<typeof getOptionValues>, isDarkMode: boolean): string => {
 ...
 */
const buildFocusEffects = (focusEffect: string | undefined, values: ReturnType<typeof getOptionValues>, isDarkMode: boolean): string => {
  ...
  switch (focusEffect) {
    ...
    case 'glow':
      focusStyles = `
        &:focus-visible {
          outline: none;
          box-shadow: 0 0 8px ${withAlpha(values.focusColor, 0.5)}, 0 0 0 2px ${withAlpha(values.focusColor, 0.2)};
        }
      `;
      break;
    case 'ring':
      focusStyles = `
        &:focus-visible {
          outline: none;
          box-shadow: 0 0 0 2px ${isDarkMode ? '#000000' : '#ffffff'}, 0 0 0 4px ${withAlpha(values.focusColor, 0.5)};
        }
      `;
      break;
    case 'border':
 ...
 */
const buildDisabledEffects = (disabledEffect: string | undefined, values: ReturnType<typeof getOptionValues>, isDarkMode: boolean): string => {
