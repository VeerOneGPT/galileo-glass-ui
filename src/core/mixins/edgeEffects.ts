...
export const glassEdge = (props: EdgeEffectProps, theme?: ThemeContext) => {
  ...
  if (props.edges && props.edges.length > 0) {
    const borderProps = props.edges.map(edge => {
      switch (edge.toLowerCase()) {
        case 'top': return `border-top: ${edgeWidth}px ${useInsetStyle ? 'inset' : 'solid'} ${edgeColor}${Math.round(edgeOpacity * 255).toString(16)};`;
        case 'right': return `border-right: ${edgeWidth}px ${useInsetStyle ? 'inset' : 'solid'} ${edgeColor}${Math.round(edgeOpacity * 255).toString(16)};`;
        case 'bottom': return `border-bottom: ${edgeWidth}px ${useInsetStyle ? 'inset' : 'solid'} ${edgeColor}${Math.round(edgeOpacity * 255).toString(16)};`;
        case 'left': return `border-left: ${edgeWidth}px ${useInsetStyle ? 'inset' : 'solid'} ${edgeColor}${Math.round(edgeOpacity * 255).toString(16)};`;
        default: return '';
      }
    }).join('\n');
    
    return css`${borderProps}`;
  }
