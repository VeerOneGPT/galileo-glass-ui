...
const createGradientBackground = (
  ...
  const { baseColor, secondaryColor, tertiaryColor, opacity, angle = 135 } = options;

  // Determine colors
  const opacityValue = getOpacityValue(opacity);
  const firstColor = baseColor || (isDarkMode ? 'rgba(30, 41, 59, 1)' : 'rgba(219, 234, 254, 1)');
  const secondColor =
    secondaryColor || (isDarkMode ? 'rgba(15, 23, 42, 1)' : 'rgba(241, 245, 249, 1)');

  // Simple two-color gradient
  if (!tertiaryColor) {
    return `
      background: linear-gradient(${angle}deg, ${withAlpha(firstColor, opacityValue)}, ${withAlpha(
      secondColor,
      opacityValue
    )});
    `;
  ...
  return `
    ...
    secondColor,
    opacityValue
  )}, ${withAlpha(thirdColor, opacityValue)});
  `;
};
  ...
  switch (patternType) {
    case 'dots':
      patternCss = `
        ...
        } ${densityValue * 100}%, transparent ${densityValue * 100 + 1}%);
    ...
    case 'lines':
      patternCss = `
        ...
        isDarkMode ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.03)'
      } 75%, ${isDarkMode ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.03)'}),
    ...
    case 'waves':
      patternCss = `
        background-color: ${withAlpha(color, opacityValue)};
        background-image: repeating-radial-gradient(circle at 0 0, transparent 0, ${withAlpha(
          color,
          opacityValue
        )} ${40 / densityValue}px), 
                          repeating-linear-gradient(${
                            isDarkMode ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.03)'
...
const createAnimatedBackground = (
  ...
  return `
    background: linear-gradient(-45deg, ${withAlpha(firstColor, opacityValue)}, ${withAlpha(
    secondColor,
    opacityValue
  )}, ${withAlpha(firstColor, opacityValue * 0.8)}, ${withAlpha(secondColor, opacityValue * 0.9)});
...
export const backgroundEffects = (options: BackgroundEffectsOptions) => {
  const { type = 'gradient', blur, themeContext } = options;
