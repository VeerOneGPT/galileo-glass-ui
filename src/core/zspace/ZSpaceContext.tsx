...
export const ZSpaceProvider: React.FC<ZSpaceProviderProps> = ({
  ...
  const getZDepth = useCallback(
    (depth: ZDepth | number): number => {
  ...
  const contextValue = useMemo<ZSpaceContextValue>(
    ...
    [baseZ, perspectiveValue, animationsEnabledValue, getZIndex, getZDepth, getTransformCSS]
  );

  return <ZSpaceContext.Provider value={contextValue}>{children}</ZSpaceContext.Provider>;
};
