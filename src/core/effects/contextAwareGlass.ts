...
const applyAdaptationMode = (
  ...
  switch (mode) {
    ...
    default:
      ...
      if (options.contentType === 'text' || options.contentType === 'control' || options.contentType === 'data') {
        result.opacity = content.adjustedOpacity * 0.7 + background.adjustedOpacity * 0.3;
        result.blurStrength = content.adjustedBlurStrength * 0.7 + background.adjustedBlurStrength * 0.3;
      ...
      else if (options.backgroundType === 'complex' || options.backgroundType === 'image' || options.backgroundType === 'dynamic') {
        result.opacity = content.adjustedOpacity * 0.3 + background.adjustedOpacity * 0.7;
        result.blurStrength = content.adjustedBlurStrength * 0.3 + background.adjustedBlurStrength * 0.7;
        result.borderOpacity = (content.adjustedBorderOpacity + background.adjustedBorderOpacity) / 2;
      }
 ...
 */
const createContrastEnhancement = (
  isDarkMode: boolean
): ReturnType<typeof css> => {
  return cssWithKebabProps`
    ${isDarkMode ? `
 ...
 */
export const contextAwareGlass = (options: ContextAwareGlassOptions = {}): ReturnType<typeof css> => {
  ...
  const normalizedReadabilityPriority = readabilityPriority || DEFAULT_CONTEXT_OPTIONS.readabilityPriority!;
  const normalizedAestheticPriority = aestheticPriority || DEFAULT_CONTEXT_OPTIONS.aestheticPriority!;
  ...
  const adjustedTintColor = withAlpha(bgTintColor || '#ffffff', (tintIntensity || 0.1) * finalOpacity);
