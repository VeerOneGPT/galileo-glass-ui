...
export const useOrchestration = (options: OrchestrationOptions) => {
  ...
  const processedStages = useMemo(() => {
    ...
    if (pattern !== 'custom') {
      stagesCopy = applyOrchestrationPattern(stagesCopy, pattern, staggerDelay, gestaltOptions);
    }
    ...
    return stagesCopy;
  }, [stages, pattern, staggerDelay, gestaltOptions, shouldReduceMotion, optimizeForPerformance]);
  ...
  function applyGestaltPattern(
    ...
    const { primaryRelationship, baseDelay = 50, maxDelay = 1000, proximityThreshold = 100 } = opts;
  ...
  const updateProgress = useCallback(() => {
    ...
    const completedCount = Object.values(stageStatesRef.current).filter(
      ...
      adjustedStages.forEach(stage => {
        ...
        if (stageState && stageState.startTime !== null && stageState.endTime === null) {
          ...
          stageTimers.current[`${stage.id}-end`] = setTimeout(() => {
            ...
            setActiveStages(prev => prev.filter(id => id !== stage.id));
            setCompletedStages(prev => (prev.includes(stage.id) ? prev : [...prev, stage.id]));
