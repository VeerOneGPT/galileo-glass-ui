...
export const usePhysicsInteraction = <T extends HTMLElement = HTMLElement>(
  ...
  const prefersReducedMotion =
    options.reducedMotion !== undefined ? options.reducedMotion : systemPrefersReducedMotion;
  ...
  const adjustedMaxDisplacement = prefersReducedMotion
    ? Math.min(maxDisplacement * 0.2, 10)
    : maxDisplacement;
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (elementRef.current) {
        ...
        // Calculate distance
        const distance = calculateDistance(mousePositionRef.current, elementPositionRef.current);
    ...
    (
      ...
      switch (type) {
        ...
        case 'magnetic':
          ...
          const magneticFactor = Math.min(
            strength,
            (strength * maxDistance) / (distance * distance + 0.1)
          );
        ...
        case 'vortex':
          ...
          force.x =
            Math.cos(spiralAngle) * radiusFactor * distance * 0.1 +
            Math.cos(spiralAngle + Math.PI / 2) * tangentialFactor * distance * 0.1;
          force.y =
            Math.sin(spiralAngle) * radiusFactor * distance * 0.1 +
  ...
  const applyAirResistance = useCallback((velocity: PhysicsVector, drag: number): PhysicsVector => {
    // Calculate velocity magnitude
    const speed = Math.sqrt(
      velocity.x * velocity.x + velocity.y * velocity.y + (velocity.z || 0) * (velocity.z || 0)
    );
  ...
  const applyBounds = useCallback(
    (position: PhysicsVector): PhysicsVector => {
      ...
      if (
        bounds.zNear !== undefined &&
        newPosition.z !== undefined &&
  ...
  const updateAnimation = useCallback(
    (timestamp: number) => {
      ...
      const effectiveEasingFunction =
        opts.easingFunction || easingFunction || easingFunctions.easeOutQuad;
        ...
        if (distance < effectiveRadius) {
          ...
          if (effectiveAffectsRotation) {
            if (['orbit', 'vortex'].includes(effectiveType)) {
              ...
              targetRotation =
                (distanceX / effectiveRadius) *
                adjustedRotationAmplitude *
                effectiveRotationAmplitude;
            }
          }
          ...
          if (effectiveAffectsScale) {
            if (effectiveType === 'repel') {
              ...
              targetScale =
                1 -
                (1 - distance / effectiveRadius) * adjustedScaleAmplitude * effectiveScaleAmplitude;
            } else {
              // Standard scaling
              targetScale =
                1 +
      ...
      if (effectiveEasingFunction && typeof effectiveEasingFunction === 'function') {
        ...
        currentStateRef.current.rotation +=
          (targetRotation - currentStateRef.current.rotation) * easingAmount;
        currentStateRef.current.scale +=
          (targetScale - currentStateRef.current.scale) * easingAmount;
      } else {
        ...
        currentStateRef.current.rotation +=
          (targetRotation - currentStateRef.current.rotation) * effectiveEaseFactor;
        currentStateRef.current.scale +=
          (targetScale - currentStateRef.current.scale) * effectiveEaseFactor;
      }
      ...
      const isMoving =
        ...
        (currentStateRef.current.velocity.z !== undefined &&
          Math.abs(currentStateRef.current.velocity.z) > 0.1);
  ...
  const update = useCallback(
    (newOptions: Partial<PhysicsInteractionOptions>) => {
      optionsRef.current = { ...optionsRef.current, ...newOptions };

      // Update spring model if mass or stiffness changed
      if (
        newOptions.mass !== undefined ||
        newOptions.stiffness !== undefined ||
        newOptions.dampingRatio !== undefined
      ) {
        ...
        const newDampingRatio =
          newOptions.dampingRatio ?? optionsRef.current.dampingRatio ?? dampingRatio;
