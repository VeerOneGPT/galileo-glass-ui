...
const getAdaptedComplexity = (
  ...
  const complexityMap: Record<
    AnimationComplexity,
    Record<DeviceCapabilityTier, AnimationComplexity>
  > = {
...
const getAdaptedDuration = (
  ...
  if (complexity === AnimationComplexity.ENHANCED || complexity === AnimationComplexity.COMPLEX) {
...
const getOptimizedProperties = (
  ...
  const hasTransform = animatedProperties.some(
    prop =>
      prop === 'transform' ||
  ...
  const hasFilter = animatedProperties.some(
    prop =>
      prop === 'filter' ||
...
const createWillChangeCSS = (performance: AnimationPerformance): string => {
  ...
  const willChangeProps = Array.from(
    new Set([...performance.properties, ...(performance.gpuAccelerated ? ['transform'] : [])])
  ).join(', ');
...
export const optimizedAnimation = (options: OptimizedAnimationOptions) => {
  ...
  const performance = getOptimizedProperties(animatedProperties, adaptedComplexity, forceGPU);
  ...
  if (
    ...
    if (
      (!isEssential && deviceTier === DeviceCapabilityTier.MINIMAL) ||
...
export const useOptimizedAnimation = (options: OptimizedAnimationOptions) => {
  ...
  const adaptedDuration = useMemo(() => {
    return options.adaptToCapabilities
      ? getAdaptedDuration(options.duration || 0.3, adaptedComplexity, deviceCapabilityTier)
