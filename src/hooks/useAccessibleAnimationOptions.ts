 ...
 */
export const useAccessibleAnimationOptions = (baseOptions: AnimationOptions): AnimationOptions => {
  // Get motion settings
  const { prefersReducedMotion, useAlternativeAnimations, disableAnimations, durationMultiplier } =
    useMotionSettings();
  ...
  const accessibleOptions = useMemo(() => {
    ...
    if (prefersReducedMotion || useAlternativeAnimations) {
      ...
      const alternativeAnimation =
        baseOptions.secondaryAnimation ||
        REDUCED_MOTION_ALTERNATIVES[
          baseOptions.animation.toString() as keyof typeof REDUCED_MOTION_ALTERNATIVES
        ] ||
