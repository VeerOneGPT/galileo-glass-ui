...
import { useOptimizedAnimation, useGlassPerformance, AnimationComplexity } from '../hooks';
...
const PerformanceIndicator = styled.div<{ status: 'good' | 'warning' | 'poor' }>`
  ...
  color: ${({ status }) => {
    switch (status) {
      case 'good':
        return 'rgba(39, 103, 73, 1)';
      case 'warning':
        return 'rgba(124, 58, 13, 1)';
      case 'poor':
        return 'rgba(155, 28, 28, 1)';
    }
  }};
`;
...
export const PerformanceOptimizedGlass: React.FC = () => {
  ...
  return (
    <div>
      ...
      <p>
        This example demonstrates how to use the performance hooks to create optimized glass UI
      ...
      <div ref={containerRef}>
        <OptimizedGlassCard complexity={complexity}>
          ...
          <p>
            This card uses optimized animations that adapt to your device capabilities. The current
            complexity level is: <strong>{complexity}</strong>
          </p>
          {optimizationLevel > 0 && (
            <div
              style={{
        ...
        <OptimizedGlassCard complexity={complexity}>
          ...
          <p>
            This card also uses the same optimization settings to ensure consistent performance
